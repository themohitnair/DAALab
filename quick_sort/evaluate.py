...
def get_mean_time(sort_func: callable, sizes: list[int] = list(range(500, 10500, 500)), iterations: int = 100) -> list[float]:
    mean_times = []
    for size in sizes:
        total_time = 0.0
...
def main():
    quicksort_mean_times = get_mean_time(quick_sort)
    plot("Quick Sort", quicksort_mean_times)
